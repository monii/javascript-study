...
`setHours`와 같은 메서드는 여러개의 날짜 구성요소를 동시에 설정할 수 있는데, 메서드의 인수에 없는 구성요소는 변경되지 않습니다.
...
```javascript
let date = new Date(2013, 0, 32); // 2013년 1월 32일은 없죠.
alert(date); // 자동으로 2013년 2월 1일이 출력됩니다.
```

입력 받은 날짜 구성요소가 범위를 벗어나면 초과분은 자동으로 다른 날짜 구성요소에 배분됩니다.

'2016년 2월 28일'의 이틀 뒤 날짜를 구하고 싶다고 가정해봅시다. 답은 3월 2일 혹은 3월 1일(윤년)이 될 텐데, 2016년이 윤년인지 아닌지 생각할 필요 없이 단순히 이틀은 더해주기만 하면 답을 구할수 있습니다. 나머지 작업은 `Date`객체가 알아서 처리해주기 때문입니다!
...
date.setDate(0); // 일의 최솟값을 1이므로 0을 입력하면 전 달의 마지막 날을 설정한 것과 같은 효과가 나타납니다.
...
```javascript
let date = new Date();
alert(+date); // 타임 스탬프(date.getTime()를 호출한 것과 동일함)
```

이를 응용하면 날짜에 마이너스 연산자를 적용해 밀리초 기준 시차를 구할 수 있습니다.
시차를 측정해 나만의 스톱워치를 만들어 봐요.
...
자바스크립트를 사용해 게임을 구현한다던가 전문분야의 애플리케이션을 구현할 때와 같이 성능이 중요한 경우에 `Date.now()`가 자주 활용됩니다. 물론 편의를 위해 활용되기도 합니다.
...
두 날짜의 차이를 계산해주는 함수 두 개가 있는데, 어느 함수의 성능이 더 좋은지 알아낸다고 가정해봅시다.
...
연속해서 함수를 아주 많이 호출한 후, 실제 연산이 종료되는데 걸리는 시간을 비교하면 두 함수의 성능을 비교할 수 있을 겁니다. `diffSubtract`와 `diffGetTime`는 아주 간단한 함수이기 때문에 유의미한 시차를 구하려면 각 함수를 최소한 십만 번 호출해야 합니다.
...
형 변환이 없어서 엔진 최적화에 드는 자원이 줄어 들어 `getTime()`을 이용한 방법이 훨씬 빠릅니다.

벤치마크로 쓴 뭔가를 구하긴 했는데, 이 벤치마크는 그다지 좋은 벤치마크가 아닙니다.
...
모던 자바스크립트 엔진은 아주 많이 실행된 코드인 'hot code'를 대상으로 최적화를 수행합니다(실행 횟수가 적은 코드는 최적화할 필요가 없죠). 위 예시에서 bench를 첫 번쨰 실행했을 때는 최적화가 잘 적용되지 않기 때문에 아래 코드처럼 메인 반복문을 실행하기 전에 예열용(heat-up)으로 bench를 실행할 수 있습니다.
...
for (let i = 0; i < 10; i++) {
  time1 += bench(diffSubtract);
  tiem2 += bench(diffGetTime);
}
```

> #### 세밀한 벤치마킹을 할 땐 주의하세요
> 
> 모던 자바스크립트 엔진은 최적화를 많이합니다. 이로 인해 '만들어진 테스트'가 '실제사례'와는 결과가 다를 수 있습니다. 특히 연산자, 내장 함수와 같이 아주 작은 것일수록 더 결과가 다를 수 있습니다. 그러나 진지하게 성능을 이해하고 싶다면 자바스크립트 엔진이 어떻게 동작하는지 공부하는 것을 추천합니다. 그럼 아마 세밀한 벤치마킹을 할 필요가 없어질 거에요.
> 
> <a href="http://mrale.ph">http://mrale.ph</a>에서 V8 엔진을 설명한 좋은 글들을 볼 수 있습니다.

### Date.parse와 문자열

메서드 <a href="https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Global_Objects/Date/parse">Date.parse(str)</a>를 사용하면 문자열에서 날짜를 읽어올 수 있습니다.
...
위 조건을 만족하는 문자열을 대상으로 `Date.parse(str)`를 호출하면 문자열과 대응하는 날짜의 타임스탬프가 반환됩니다. 문자열의 형식이 조건에 맞지 않은 경우에는 `NaN`이 반환됩니다.
